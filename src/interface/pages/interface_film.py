...
def ajouter_a_watchlist(film_id):
    ...
    try:
        ...
        if ajout_response.status_code == 200:
            ...
            print(f"❌ Erreur lors de l'ajout du film : {ajout_response.json().get('detail', 'Erreur inconnue')}")
            return
    ...
    try:
        plateformes_response = requests.post(
            f"{LIEN_API}/films/ajouter_plateformes",
            json={"id_film": film_id}
        )
        if plateformes_response.status_code == 200:
            print(f"✅ Les plateformes pour le film (ID: {film_id}) ont été associées avec succès.")
        else:
            print(f"❌ Erreur lors de l'association des plateformes : {plateformes_response.json().get('detail', 'Erreur inconnue')}")

    except requests.exceptions.RequestException as e:
        print(f"❌ Une erreur s'est produite lors de l'association des plateformes : {e}")
...
def afficher_films_pagination(films):
    ...
    while True:
        ...
        options = ["Sélectionner un film", "Page suivante", "Page précédente", "Retour au menu principal"]
        if page == 0:
            ...
            questions = [
                {
                    ...
                    "validate": lambda result: result.isdigit() or result.lower() == "retour" or "Veuillez entrer un ID valide ou 'Retour'.",
                }
            ]
