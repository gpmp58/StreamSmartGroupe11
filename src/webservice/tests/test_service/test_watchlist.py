...
@pytest.mark.parametrize(
    "nom_watchlist , id_utilisateur, list_film , "
    "id_watchlist , erreur, message_erreur",
    [
        (
            {"watchlist_alice": ""},
            123,
...
def test_watchlist_init_echec(
    ...
    with pytest.raises(erreur, match=re.escape(message_erreur)):
        Watchlist(
            nom_watchlist, id_utilisateur, list_film, id_watchlist
        )
...
def test_verifier_utilisateur_succes():
    ...
    watchlist = Watchlist(
        "watchlist_alice", 1234, ["film1", "film2"], 1290
    )
    assert watchlist.verifier_utilisateur(utilisateur)
...
def test_verifier_utilisateur_echec(
    utilisateur, erreur, message_erreur
):
    # GIVEN
    watchlist = Watchlist(
        "watchlist_alice", 123, ["film1", "film2"], 1290
    )
    with pytest.raises(erreur, match=re.escape(message_erreur)):
