...
def test_creer_nouvelle_watchlist_succes(mock_creer_watchlist):
    ...
    mock_creer_watchlist.return_value = True  # Simule un succès lors de la création en DB
...
def test_supprimer_watchlist_succes(mock_supprimer_watchlist):
    ...
    watchlist_valid = Watchlist(id_watchlist=1, nom_watchlist="MaWatchlist", id_utilisateur=1)
...
def test_supprimer_watchlist_echec(mock_supprimer_watchlist):
    ...
    # Création d'une watchlist valide
    watchlist_valid = Watchlist(id_watchlist=2, nom_watchlist="AutreWatchlist", id_utilisateur=1)
...
def test_ajouter_film_succes(mock_film_deja_present, mock_verifier_film_existe, mock_ajouter_film_DAO):
    """
    ...
    film = Film(id_film=1)
    watchlist = Watchlist(id_watchlist=1, nom_watchlist="Ma Watchlist", id_utilisateur=1)
...
@patch('src.webservice.dao.watchlist_dao.WatchlistDao.ajouter_film_DAO')
@patch('src.webservice.dao.watchlist_dao.WatchlistDao.film_deja_present')
def test_ajouter_film_inexistant_ajoute_succes(mock_film_deja_present, mock_ajouter_film, mock_verifier_film_existe, mock_ajouter_film_DAO):
    """
...
@patch('src.webservice.dao.watchlist_dao.WatchlistDao.verifier_film_existe')
@patch('src.webservice.dao.watchlist_dao.WatchlistDao.film_deja_present')
def test_ajouter_film_echec(mock_film_deja_present, mock_verifier_film_existe, mock_ajouter_film_DAO):
    """
...
def test_supprimer_film_succes(mock_supprimer_film_DAO, mock_film_deja_present):
    ...
    service = WatchlistService()
    film = Film(id_film=1)
    watchlist = Watchlist(id_watchlist=1, nom_watchlist="Test Watchlist", id_utilisateur=1)
...
def test_sauvegarder_watchlist_id_invalide():
    # Initialisation
    service = WatchlistService()
    watchlist = Watchlist(id_watchlist=None, nom_watchlist="Test Watchlist", id_utilisateur=1)
    ...
    assert resultat == []
    mock_logging_error.assert_called_once_with("Erreur: l'identifiant de la watchlist est invalide ou manquant.")
