...
class Watchlist:
    ...
    def __init__(self, nom_watchlist: str, id_utilisateur, list_film=None, id_watchlist=None):
        ...
        if not isinstance(nom_watchlist, str):
            raise TypeError(f"nom_watchlist doit être une chaîne de caractères, mais reçu {type(nom_watchlist).__name__}")
        
        if not isinstance(id_utilisateur, int):
            raise TypeError(f"id_utilisateur doit être un entier, mais reçu {type(id_utilisateur).__name__}")
        
        if list_film is not None and not isinstance(list_film, list):
            raise TypeError(f"list_film doit être une liste ou None, mais reçu {type(list_film).__name__}")

        if id_watchlist is not None and not isinstance(id_watchlist, int):
            raise TypeError(f"id_watchlist doit être un entier ou None, mais reçu {type(id_watchlist).__name__}")
